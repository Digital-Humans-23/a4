...
from scripts.gen_floor import LISSTGEMPGenFloorOP, MinHeap, MPTNodeTorch

# DEVICE=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
DEVICE=torch.device('cpu')

class LISSTGEMPGenLocationOP(LISSTGEMPGenFloorOP):
    def eval_quality(self, motion_primitive):
        ...
        feet_height = motion_primitive.view(-1, 1, 31, 9)[:, :, feet_joints, 2]
        root_idx = self.model.body_model.joint_names.index('root')
        location_goal = self.location_goal  # [2, ]
        quality_floor = 0
        quality_location = 0
        ...
        quality_dict = {
            'floor': quality_floor,
            'location': quality_location,
            'all': quality_floor + quality_location
        }
    ...
    def search(self, J_seed, J_shape, max_depth=10,
        ...
        for depth in range(1, max_depth):
            ...
            if mp_heap_prev.data[0].quality_dict['location'] > 0.9:
                print('reach goal')
                break
        ...
        for key in quality_dict_list[0].keys():
            quality_log[key] = torch.stack(
                [quality_dict[key] for quality_dict in quality_dict_list]).detach().cpu().numpy()
        print(quality_log)
...
if __name__ == '__main__':
    ...
    """model and testop"""
    testop = LISSTGEMPGenLocationOP(mover_config.modelconfig, testcfg)
    testop.build_model()
    ...
    testop.generate(data, J_shape,
                    save_dir='results/location_search',
                    location_goal=torch.tensor([1.0, 1.0]).to(device=DEVICE),
                    n_seqs=1, max_depth=30,
                    n_gens_1frame=128, n_gens_2frame=16, num_expand=8)
    testop.generate(data, J_shape,
                    save_dir='results/location_random',
                    location_goal=torch.tensor([1.0, 1.0]).to(device=DEVICE),
                    n_seqs=1, max_depth=30,
